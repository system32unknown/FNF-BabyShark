...
class Hitmark extends FlxSprite {
    ...
    override function update(elapsed:Float) {
		...
		decayTime += elapsed;
		var s = Conductor.crochet * .001;

		var decayScale = 1 - (decayTime / (s * 8)); // 8 beats to decay
		scale.y = decayScale;
		alpha = baseAlpha * decayScale;
		if (decayScale <= 0) kill();
    }
}

/**
    * Hit Error Bar by Troll Engine Team, But Ported to Alter Engine.
    * I'll remove that if you want to take down request.
    * @author Altertoriel
*/
class Hitbar extends FlxSpriteGroup {
    ...
    @:isVar public var currentAverage(get, null):Float = 0;
	...
	public function new(?x:Float, ?y:Float){
		...
		mainBar = new FlxSprite().makeGraphic(Std.int(hitbarPxPerMs * ((ClientPrefs.getPref('safeFrames') / 60) * 1000)), hitbarHeight, FlxColor.BLACK);
		mainBar.alpha = 0.5;
		add(mainBar);

		var epicWindow = new FlxSprite().makeGraphic(Std.int(hitbarPxPerMs * ClientPrefs.getPref('epicWindow')), hitbarHeight, 0xFFE367E5);
		epicWindow.alpha = 0.6;
		var sickWindow = new FlxSprite().makeGraphic(Std.int(hitbarPxPerMs * ClientPrefs.getPref('sickWindow')), hitbarHeight, 0xFF00A2E8);
		sickWindow.alpha = 0.6;
		var goodWindow = new FlxSprite().makeGraphic(Std.int(hitbarPxPerMs * ClientPrefs.getPref('goodWindow')), hitbarHeight, 0xFFB5E61D);
		goodWindow.alpha = 0.6;
		var badWindow = new FlxSprite().makeGraphic(Std.int(hitbarPxPerMs * ClientPrefs.getPref('badWindow')), hitbarHeight, FlxColor.BLACK);
		badWindow.alpha = 0.6;
		...
		averageIndicator = new FlxSprite(Paths.image("hitbarAverage"));
    ...
    override function update(elapsed:Float) {
		...
		markGroup.forEachAlive((obj:Hitmark) -> {
			obj.baseAlpha = alpha;
			obj.visible = visible;
		});

		markGroup.forEachDead((obj:Hitmark) -> markGroup.remove(obj, true));
        super.update(elapsed);
