...
class CreditsPageSchema {
     ...
     * @return FlxGraphic
     */
	public function getHeaderGraphic():FlxGraphic {
		var str:String = 'credits/header/$icon';
		if(!Paths.exists(Paths.getPath('images/$str.png', IMAGE))) str = 'credits/header/missing_icon';
...
class CreditSchema {
	...
	public function new(name:String, icon:String = null, role:String = null, url:Null<String> = null, color:Null<FlxColor> = null, pronouns:Null<String> = null, ?antialiasing:Null<Bool> = null):Void {
		...
		this.url = url;
		this.color = color;
		if (antialiasing == null) antialiasing = !icon.endsWith("-pixel");
	...
	public function getIconGraphic():FlxGraphic {
		...
		if(!Paths.exists(Paths.getPath('images/$str.png', IMAGE))) str = 'credits/missing_icon';
		return Paths.image('images/$str.png');
	}
}