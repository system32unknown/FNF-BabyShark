...
class NoteSplash extends FlxSprite {
				...
				if ((note == null || !note.noteSplashData.useGlobalShader) || inEditor) {
					...
					if (colors != null) {
						for (i in 0...colors.length) {
							...
							var arr:Array<FlxColor> = (PlayState.isPixelStage ? Settings.data.arrowRGBPixel : Settings.data.arrowRGB)[_noteData];
	...
	function checkForAnim(anim:String):Bool {
	...
	override function update(elapsed:Float) {
		...
		if (babyArrow != null) {
			...
			if (copyY) y = babyArrow.y - Note.originalHeight;
		}
	...
	public function killLimit(targetId:Int = -1):Void {
		...
		} catch (e:Dynamic) Logs.warn("something went wrong? " + e.message);
		kill();
	}
}