...
class AchievementPopup extends openfl.display.Sprite {
	...
	public function new(name:String, onFinish:Void->Void) {
		...
		// achievement icon
		var graphic = Paths.image('achievements/$name');
		...
		graphics.beginBitmapFill(image, new Matrix(sizeX / image.width, 0, 0, sizeY / image.height, imgX, imgY), false, ClientPrefs.getPref('Antialiasing'));
