...
class CustomModifierScript
{
    ...
    public function new(scriptStr:String)
    {
        ...
        try {
            ...
            lime.app.Application.current.window.alert(e.message, 'Error on custom mod .modchart!');
            return;
        }
        init();
    }
    private function init()
    {
        ...
        interp.variables.set('Note', game.Note);
    }
    public function call(event:String, args:Array<Dynamic>)
    {
        ...
        if (interp.variables.exists(event)) //make sure it exists
        {
            try {
                ...
                else interp.variables.get(event)(); //if function doesnt need an arg
            } catch(e) {
                lime.app.Application.current.window.alert(e.message, 'Error on custom mod .modchart!');
            }
        }
