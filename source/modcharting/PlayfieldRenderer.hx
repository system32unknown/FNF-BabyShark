...
class PlayfieldRenderer extends FlxSprite // extending flxsprite just so i can edit draw
{
	...
	function getDataForStrum(i:Int, pf:Int) {
	...
	function createDataFromNote(noteIndex:Int, playfieldIndex:Int, curPos:Float, noteDist:Float, incomingAngle:Array<Float>) {
	...
	function getNoteCurPos(noteIndex:Int, strumTimeOffset:Float = 0) {
	...
	function getLane(noteIndex:Int) {
	...
	function getNoteDist(noteIndex:Int) {
		var noteDist = -.45;
	...
	function getSustainPoint(noteData:NotePositionData, timeOffset:Float):NotePositionData {
		...
		var finalNotePos = ModchartUtil.calculatePerspective(new Vector3D(noteData.x + (daNote.width / 2) + ModchartUtil.getNoteOffsetX(daNote, instance), noteData.y + (NoteMovement.arrowSizes[noteData.lane] / 2), noteData.z * 0.001), ModchartUtil.defaultFOV * (Math.PI / 180), -(daNote.width / 2), yOffsetThingy - (NoteMovement.arrowSizes[noteData.lane] / 2));
