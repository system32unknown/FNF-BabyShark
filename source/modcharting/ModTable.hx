...
class ModTable {
	...
	var instance:ModchartMusicBeatState = null;
	var renderer:PlayfieldRenderer = null;
	...
	var table:Array<Array<Array<Modifier>>> = [];
	...
	public function remove(tag:String):Void {
		if (modifiers.exists(tag)) modifiers.remove(tag);
	}
	...
	public function resetMods():Void {
		for (mod in modifiers) mod.reset();
	}
	...
	public function applyStrumMods(noteData:NotePositionData, lane:Int, pf:Int):Void {
		if (table[pf] != null && table[pf][lane] != null) {
			...
			for (mod in modList) mod.getStrumPath(noteData, lane, pf);
		}
	}
	...
	public function applyNoteMods(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int):Void {
		if (table[pf] != null && table[pf][lane] != null) {
			var modList:Array<Modifier> = table[pf][lane];
			for (mod in modList) mod.getNotePath(noteData, lane, curPos, pf);
		}
	}
	...
	public function applyNoteDistMods(noteDist:Float, lane:Int, pf:Int):Float {
		if (table[pf] != null && table[pf][lane] != null) {
			var modList:Array<Modifier> = table[pf][lane];
			for (mod in modList) noteDist = mod.getNoteDist(noteDist, lane, 0, pf);
		}
		return noteDist;
	}
	...
	public function applyCurPosMods(lane:Int, curPos:Float, pf:Int):Float {
		if (table[pf] != null && table[pf][lane] != null) {
			var modList:Array<Modifier> = table[pf][lane];
			for (mod in modList) curPos = mod.getNoteCurPos(lane, curPos, pf);
		}
		return curPos;
	}
	...
	public function applyIncomingAngleMods(lane:Int, curPos:Float, pf:Int):Array<Float> {
		...
		if (table[pf] != null && table[pf][lane] != null) {
			var modList:Array<Modifier> = table[pf][lane];
			for (mod in modList) {
				var ang:Array<Float> = mod.getIncomingAngle(lane, curPos, pf); // need to get incoming angle before
	...
	public function tweenModifier(modifier:String, val:Float, time:Float, ease:String, beat:Float, ?tag:String = null) {
		...
		if (modifiers.exists(modifier)) {
			var easefunc = LuaUtils.getTweenEaseByString(ease);
			...
			time /= renderer.speed;
			var tween:FlxTween = renderer.createTween(modifiers.get(modifier), {currentValue: val}, time, {
				ease: easefunc,
				onComplete: (twn:FlxTween) -> PlayState.instance.callOnScripts("onModifierComplete", [tag ?? modifier])
			});
			if (Conductor.songPosition > ModchartUtil.getTimeFromBeat(beat)) @:privateAccess { // skip to where it should be i guess??
				tween._secondsSinceStart += ((Conductor.songPosition - ModchartUtil.getTimeFromBeat(beat)) * .001);
				tween.update(0);
			}
			if (renderer.editorPaused) tween.active = false;
		}
	}
	...
	public function tweenModifierSubValue(modifier:String, subValue:String, val:Float, time:Float, ease:String, beat:Float, ?tag:String = null) {
		...
		if (modifiers.exists(modifier)) {
			if (modifiers.get(modifier).subValues.exists(subValue)) {
				var easefunc = LuaUtils.getTweenEaseByString(ease);
				var tag:String = modifier + ' ' + subValue;

				var startValue:Float = modifiers.get(modifier).subValues.get(subValue).value;
				...
				var tween = renderer.createTweenNum(startValue, val, time, {
					...
					onComplete: function(twn:FlxTween) {
						if (modifiers.exists(modifier)) modifiers.get(modifier).subValues.get(subValue).value = val;
