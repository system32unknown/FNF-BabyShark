...
@:access(funkin.objects.NoteSplash)
class NoteSplashEditorState extends MusicBeatState {
	...
	function addAnimTab() {
		...
		animDropDown = new PsychUIDropDownMenu(-155, 57, [""], (id:Int, name:String) -> {
		...
		addButton = new PsychUIButton(20, 185, "Add/Update", function() {
			...
			var conf:NoteSplashAnim = config.animations.get(name_input.text);
		...
		reloadImage = function() {
			...
			var image:flixel.graphics.FlxGraphic = Paths.image(imageSkin);
			if (image == null) {
	...
	function addPropertiesTab() {
		...
		ui.add(new PsychUIButton(20, 130, "Save", saveSplash));
	...
	function addShadersTab() {
		var tab:FlxSpriteGroup = shaderUI.getTab("Shader").menu;
		...
		var green:PsychUINumericStepper = new PsychUINumericStepper(60, 50, 1, redShader[1], 0, 255, 0);
		green.onValueChange = () -> {
			switch (changeShader.selectedLabel) {
		...
		var blue:PsychUINumericStepper = new PsychUINumericStepper(60, 70, 1, redShader[2], 0, 255, 0);
		blue.onValueChange = () -> {
			switch (changeShader.selectedLabel) {
		...
		function onCheck(change:Bool = true) {
			if (!defaultButton.checked) shaderUI.alpha = 1;
			else shaderUI.alpha = 0.6;

			if (change) switch (changeShader.selectedLabel) {
		...
		changeShader = new PsychUIDropDownMenu(shaderUI.x + 20, shaderUI.y + 150, ["Red", "Green", "Blue"], function(id:Int, name:String) {
			var shader:Array<Int> = switch (name) {
				case "Red": redShader;
	...
	function parseRGB() {
		...
		if (config.rgb != null)
			for (i in 0...config.rgb.length) {
				...
				var rgb:funkin.objects.NoteSplash.RGB = config.rgb[i];
				if (rgb == null) {
				...
				var colors:Array<Int> = [rgb.r, rgb.g, rgb.b];
	...
	function setConfigRGB() {
		...
		if (redEnabled) config.rgb.push({r: redShader[0], g: redShader[1], b: redShader[2]});
		else config.rgb.push(null);

		if (greenEnabled) config.rgb.push({r: greenShader[0], g: greenShader[1], b: greenShader[2]});
		else config.rgb.push(null);

		if (blueEnabled) config.rgb.push({r: blueShader[0], g: blueShader[1], b: blueShader[2]});
		else config.rgb.push(null);
	}
	...
	function onLoadComplete(_):Void {
		...
		try {
			...
			var conf:NoteSplashConfig = parseTxt(txt);
			_file = new FileReference();
			_file.addEventListener(Event.COMPLETE, onSaveComplete);
			_file.addEventListener(Event.CANCEL, onSaveCancel);
			_file.addEventListener(IOErrorEvent.IO_ERROR, onSaveError);
	...
	public static function parseTxt(content:String):NoteSplashConfig {
		...
		if (configs[1] != null && configs[1].trim() != "") {
			var newFps:Array<String> = configs[1].trim().split(" ");
