...
class BezierPathTween extends FlxTween {
		...
		if (_secondsSinceStart < delay) super.update(elapsed);
			...
			if (active)
				for (info in _propertyInfos)
					...
					} else Reflect.setProperty(info.object, info.field, bezierPath(scale,info.points));
		}
	}
	...
	function initializeVars():Void {
		...
		else throw "Unsupported properties container - use an object containing key/value pairs.";

		for (fieldPath in fieldPaths) {
			var target = _object;
			var path:Array<String> = fieldPath.split(".");
