...
class ReflectionFunctions
{
	public static function implement(funk:FunkinLua)
	{
		...
		funk.addCallback(lua, "getProperty", function(variable:String, ?allowMaps:Bool = false) {
		...
		Lua_helper.add_callback(lua, "getPropertyFromGroup", function(obj:String, index:Int, variable:Dynamic, ?allowMaps:Bool = false) {
			...
			if(split.length > 1)
				...
				realObject = Reflect.getProperty(LuaUtils.getInstance(), obj);
