...
class Vector3 {
	...
	**/
	public function lerp(goal:Vector3, alpha:Float, result:Null<Vector3> = null):Vector3 {
		if (result == null) result = new Vector3();
		result.setTo(alpha * goal.x + x * (1 - alpha), alpha * goal.y + y * (1 - alpha), alpha * goal.z + z * (1 - alpha));
		return result;
	}
	...
	**/
	...
	@:noCompletion inline function get_lengthSquared():Float {
		return x * x + y * y + z * z;
	}

	inline static function get_X_AXIS():Vector3 {
		return new Vector3(1, 0, 0);
	}

	inline static function get_Y_AXIS():Vector3 {
		return new Vector3(0, 1, 0);
	}

	inline static function get_Z_AXIS():Vector3 {
