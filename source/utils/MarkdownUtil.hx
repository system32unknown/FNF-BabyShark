...
class MarkdownUtil {
	...
	public static function createAdvancedMarkerPair(marker:String, color:FlxColor, size:Null<Float>, bold:Bool = false, italic:Bool = false, ?underline:Bool = false, ?blockIndent:Int, ?bullet:Bool):FlxTextFormatMarkerPair {
		@:privateAccess {
			var format:FlxTextFormat = new FlxTextFormat(color, bold, italic);
	...
	public static function parseWarnings(text:String):String {
		text = text.replace("&nbsp;", " ");
	...
	public static function prepareMarkdown(text:String):String {
		text = "\n" + text;
	...
	public static function parseEmote(text:String, emote:String, beginning:String, end:String, emoteEnd:String = "\n"):String {
		...
		while((index = text.indexOf(emote)) >= 0) {
			var nextIndex:Int = text.indexOf(emoteEnd, index + emote.length);
