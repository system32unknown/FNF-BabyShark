...
class DialogueCharacterEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent {
	...
	static var TIP_TEXT_MAIN:String =
	...
	static var TIP_TEXT_OFFSET:String =
	'JKLI - Move camera (Hold Shift to move 4x faster)
	\nQ/E - Zoom out/in
	\nR - Reset Camera
	\nH - Toggle Ghosts
	...
	function addAnimationsUI() {
		var tab_group:FlxSpriteGroup = UI_mainbox.getTab('Animations').menu;
	...
	function reloadCharacter() {
		for (char in [character, ghostLoop, ghostIdle]) {
			char.frames = Paths.sparrowAtlas('dialogue/' + character.jsonFile.image);
