...
enum abstract UndoAction(String) {
	var ADD_NOTE:UndoAction = 'Add Note';
	var DELETE_NOTE:UndoAction = 'Delete Note';
	var MOVE_NOTE:UndoAction = 'Move Note';
	var SELECT_NOTE:UndoAction = 'Select Note';
}
...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent {
	...
	override function create() {
		...
		if (chartEditorSave.data.customGridColors == null || chartEditorSave.data.customGridColors.length < 2) chartEditorSave.data.customGridColors = ['DFDFDF', 'BFBFBF'];
		if (chartEditorSave.data.customNextGridColors == null || chartEditorSave.data.customNextGridColors.length < 2) chartEditorSave.data.customNextGridColors = ['5F5F5F', '4A4A4A'];
		...
		vortexIndicator = new FlxSprite(gridBg.x - GRID_SIZE, FlxG.height / 2, Paths.image('editors/vortex_indicator'));
		...
		if (SHOW_EVENT_COLUMN) {
			eventIcon = new FlxSprite(0, iconY, Paths.image('editors/eventIcon'));
		...
		mustHitIndicator = flixel.util.FlxSpriteUtil.drawTriangle(new FlxSprite(0, iconY - 20).makeGraphic(16, 16, FlxColor.TRANSPARENT), 0, 0, 16);
		...
		infoBox = new PsychUIBox(infoBoxPosition.x, infoBoxPosition.y, 220, 240, ['Information']);
		...
		gameOverCharacters.sort((a:String, b:String) -> {
	...
	override function update(elapsed:Float) {
		...
		if (autoSaveCap > 0) {
			...
			if (autoSaveTime >= autoSaveCap #if debug || FlxG.keys.justPressed.NUMPADMULTIPLY #end) {
				...
				FlxTween.tween(autoSaveIcon, {alpha: 1}, 0.5, {onComplete: (_) -> FlxTween.tween(autoSaveIcon, {alpha: 0}, 0.5, {startDelay: 2})});
			}
		}

		Controls.toggleVolumeKeys(charterFocus);
			...
			if (charterFocus) { // If not typing anything
				...
				if (FlxG.keys.justPressed.A != FlxG.keys.justPressed.D && !holdingAlt) {
					...
					if (mouseSnapCheckBox.checked && FlxG.mouse.wheel != 0) {
						...
						if (FlxG.keys.pressed.W || FlxG.mouse.wheel > 0) FlxG.sound.music.time -= Conductor.crochet * speedMult * 1.5 * elapsed / curZoom;
						else if (FlxG.keys.pressed.S || FlxG.mouse.wheel < 0) FlxG.sound.music.time += Conductor.crochet * speedMult * 1.5 * elapsed / curZoom;
					}
		...
		if (charterFocus) {
			...
			if (FlxG.keys.justPressed.ENTER) {
				goToPlayState();
				return;
			} else if (FlxG.keys.pressed.CONTROL && !isMovingNotes && (FlxG.keys.justPressed.Z || FlxG.keys.justPressed.Y || FlxG.keys.justPressed.X || FlxG.keys.justPressed.C || FlxG.keys.justPressed.V || FlxG.keys.justPressed.A || FlxG.keys.justPressed.S)) {
				canContinue = false;
				if (FlxG.keys.justPressed.Z) undo();
					...
					if (selectedNotes.length > 0) {
						...
						for (note in pushedNotes) note[0] -= minTime;
					}
					...
					if (copiedNotes.length > 0 || copiedEvents.length > 0) {
						...
						selectedNotes.sort(SortUtil.byStrumTime);
					...
					var sel:Array<MetaNote> = selectedNotes;
				...
				} else if (FlxG.keys.justPressed.S) saveChart(); // Save (Ctrl + S)
			}
			...
			if (doCut || FlxG.keys.justPressed.DELETE || FlxG.keys.justPressed.BACKSPACE || (isMovingNotes && (FlxG.mouse.justPressedRight || FlxG.keys.justPressed.ESCAPE))) { // Delete button
				...
				if (FlxG.keys.justPressed.LEFT != FlxG.keys.justPressed.RIGHT) { // Lower/Higher quant
					...
					else curQuant = quantizations[Std.int(Math.min(quantizations.indexOf(curQuant) + 1, quantizations.length - 1))];
					forceDataUpdate = true;
					...
					else curZoom = zoomList[Std.int(Math.min(zoomList.indexOf(curZoom) + 1, zoomList.length - 1))];

					notes.sort(SortUtil.byStrumTime);
					var noteSec:Int = 0;
		...
		if (selectionBox.visible) {
			if (FlxG.mouse.releasedRight) {
				var sel:Array<MetaNote> = selectedNotes.copy();
				updateSelectionBox();
				if (!FlxG.keys.pressed.SHIFT && !holdingAlt) resetSelectedNotes();

				var selectionBounds:FlxRect = selectionBox.getScreenBounds(null, camUI);
				...
				for (note in curRenderedNotes) {
					...
					if (!selectedNotes.contains(note) || holdingAlt) {
						var noteBounds:FlxRect = note.getScreenBounds(null, camUI);
		...
		if (FlxG.mouse.x >= minX && FlxG.mouse.x < gridBg.x + gridBg.width) {
			...
			if (isMovingNotes) {
				...
				if (FlxG.keys.pressed.CONTROL && FlxG.mouse.justPressed) {
					...
					var closeNotes:Array<MetaNote> = curRenderedNotes.members.filter((note:MetaNote) -> {
					...
					closeNotes.sort((a:MetaNote, b:MetaNote) -> return Math.abs(a.strumTime - FlxG.mouse.y) < Math.abs(b.strumTime - FlxG.mouse.y) ? 1 : -1);

					var closest:MetaNote = closeNotes[0];
					if (closest != null && (!closest.isEvent || !lockedEvents)) {
						...
						if (noteData >= 0) {
							...
							for (num in sectionFirstEventID...events.length) {
								var event:EventMetaNote = events[num];
		...
		if (selectedNotes.length > 0) {
			...
			var sineValue:Float = .75 + Math.cos(Math.PI * noteSelectionSine * (isMovingNotes ? 8 : 2)) / 4;

			var qPress:Bool = FlxG.keys.justPressed.Q;
			var ePress:Bool = FlxG.keys.justPressed.E;
			var addSus:Float = (FlxG.keys.pressed.SHIFT ? 4 : 1) * (Conductor.stepCrochet / 2);
			if (qPress) addSus *= -1;
	...
	function hitNote(note:MetaNote) {
		...
		if (canPlayHitSound) {
			var soundToPlay:String = 'hitsounds/${Std.string(Settings.data.hitsoundTypes).toLowerCase()}';
	...
	function moveSelectedNotes(noteData:Int = 0, lastY:Float) { //This turns selected notes into moving notes
		...
		movingNotes.sort(cast SortUtil.byStrumTime);
	...
	function stopMovingNotes() { // This turns moving notes into saved notes
		var pushedNotes:Array<MetaNote> = [];
		var pushedEvents:Array<EventMetaNote> = [];
		movingNotes.forEachAlive((note:MetaNote) -> {
			if (!note.isEvent) {
				notes.push(note);
				pushedNotes.push(note);
			} else {
		...
		notes.sort(SortUtil.byStrumTime);
		events.sort(SortUtil.byStrumTime);
		movingNotes.clear();
		isMovingNotes = false;
		softReloadNotes();
	}

	function makeNoteDataCopy(originalData:Array<Dynamic>, isEvent:Bool):Array<Dynamic> {
		var dataCopy:Array<Dynamic> = originalData.copy();
		if (isEvent) {
			var eventGrp:Array<Array<Dynamic>> = cast dataCopy[1].copy();
			for (num => subEvent in eventGrp) eventGrp[num] = subEvent.copy();
			dataCopy[1] = eventGrp;
		}
		return dataCopy;
	}
	...
	function onSelectNote() {
		if (selectedNotes.length == 1) { // Only one note selected
			...
			} else updateSelectedEventText(); // Event note
	...
	function updateSelectedEventText() {
		if (selectedNotes.length == 1 && selectedNotes[0].isEvent) {
			...
			if (myEvent != null) {
				var eventName:String = myEvent[0] ?? '';
				...
				value1InputText.text = myEvent[1] ?? '';
				value2InputText.text = myEvent[2] ?? '';
			}
	...
	function createNote(note:Dynamic, ?secNum:Null<Int> = null):MetaNote {
		...
		else swagNote.setGraphicSize(0, GRID_SIZE);
	...
	function createEvent(event:Dynamic):EventMetaNote {
	...
	function softReloadNotes(onlyCurrent:Bool = false) {
		...
		function curSecFilter(note:MetaNote):Bool {
		...
		for (num => note in notes) {
			if (note != null && curSecFilter(note)) {
				...
				note.alpha = (note.strumTime >= Conductor.songPosition) ? 1 : .6;
		...
		if (!onlyCurrent) {
			if (showPreviousSection || showNextSection) {
				...
				function otherSecFilter(note:MetaNote):Bool {
					return (prevGridBg.visible && (note.strumTime >= prevMinTime && note.strumTime < prevMaxTime)) || (nextGridBg.visible && (note.strumTime >= nextMinTime && note.strumTime < nextMaxTime));
				}
	...
	function updateHeads(ignoreCheck:Bool = false):Void {
		var curSecData:Section = PlayState.SONG.notes[curSec];
		...
		if (icons.length > 1) {
			...
			if (isGfSection) (mustHitSection ? iconP1 : iconP2).changeIcon('gf');
			if (mustHitSection) mustHitIndicator.x = iconP1.x + iconP1.width / 2;
			else mustHitIndicator.x = iconP2.x + iconP2.width / 2;
		}
	...
	function addChartingTab() {
		var tab_group:FlxSpriteGroup = mainBox.getTab('Charting').menu;
		var objX:Int = 10;
		var objY:Int = 10;

		var txt:FlxText = new FlxText(objX, objY, 280, "Any options here won't actually affect gameplay!");
		...
		objY += 60;
		mouseSnapCheckBox = new PsychUICheckBox(objX, objY, 'Mouse Scroll Snap', 100, () -> chartEditorSave.data.mouseScrollSnap = mouseSnapCheckBox.checked);
		mouseSnapCheckBox.checked = chartEditorSave.data.mouseScrollSnap;

		ignoreProgressCheckBox = new PsychUICheckBox(objX + 150, objY, 'Ignore Progress Warnings', 100, () -> chartEditorSave.data.ignoreProgressWarns = ignoreProgressCheckBox.checked);
		...
		tab_group.add(new FlxText(playerVolumeStepper.x, playerVolumeStepper.y - 15, 100, 'Vocals:'));
	...
	function addDataTab() {
		var tab_group:FlxSpriteGroup = mainBox.getTab('Data').menu;
		var objX:Int = 10;
		var objY:Int = 25;
		gameOverCharDropDown = new PsychUIDropDownMenu(objX, objY, [''], (id:Int, character:String) -> {
			...
			if (changed) {
				...
				if (Paths.fileExists(textureLoad, IMAGE) || noteTextureInputText.text.trim() == '') {
					...
					for (strum in strumLineNotes) {
						...
						if (strum.width > strum.height) strum.setGraphicSize(GRID_SIZE);
						else strum.setGraphicSize(0, GRID_SIZE);
	...
	function addEventsTab() {
		var tab_group:FlxSpriteGroup = mainBox.getTab('Events').menu;
		var objX:Int = 10;
		var objY:Int = 25;
		...
		var removeButton:PsychUIButton = new PsychUIButton(objX2, objY, '-', () -> {
			genericEventButton((event:EventMetaNote) -> {
		...
		var addButton:PsychUIButton = new PsychUIButton(objX2 + 30, objY, '+', () -> {
			genericEventButton((event:EventMetaNote) -> {
		...
		var leftButton:PsychUIButton = new PsychUIButton(objX2 + 80, objY, '<', () -> genericEventButton((event:EventMetaNote) -> curEventSelected = FlxMath.wrap(curEventSelected - 1, 0, event.events.length - 1)), 20);
		var rightButton:PsychUIButton = new PsychUIButton(objX2 + 110, objY, '>', () -> genericEventButton((event:EventMetaNote) -> curEventSelected = FlxMath.wrap(curEventSelected + 1, 0, event.events.length - 1)), 20);
	...
	function addNoteTab() {
		...
		noteTypeDropDown = new PsychUIDropDownMenu(objX, objY, [], (id:Int, changeToType:String) -> {
			...
			for (note in selectedNotes) {
				...
				else note.songData.remove(note.songData[3]);
	...
	function addSectionTab() {
		...
		var tab_group:FlxSpriteGroup = mainBox.getTab('Section').menu;
		var objX:Int = 10;
		var objY:Int = 10;
		...
		altAnimSectionCheckBox = new PsychUICheckBox(objX + 200, objY, 'Alt Anim.', 80, () -> {
			var sec = getCurChartSection();
			if (sec != null) sec.altAnim = altAnimSectionCheckBox.checked;
		});

		objY += 40;
		changeBpmCheckBox = new PsychUICheckBox(objX, objY, 'Change BPM', 80, () -> {
			var sec = getCurChartSection();
			if (sec != null) {
		...
		beatsPerSecStepper.onValueChange = () -> {
		...
		var pasteButton:PsychUIButton = new PsychUIButton(objX + 100, objY, 'Paste Section', () -> pasteCopiedNotesToSection(affectNotes.checked, affectEvents.checked));
		var clearButton:PsychUIButton = new PsychUIButton(objX + 200, objY, 'Clear', () -> {
			for (note in curRenderedNotes) {
				if (note == null) continue;
		...
		objY += 32;
		var copyLastSecButton:PsychUIButton = new PsychUIButton(objX, objY, 'Copy Last Section', () -> {
			var lastCopiedNotes:Array<Dynamic> = copiedNotes;
			var lastCopiedEvents:Array<Dynamic> = copiedEvents;
		...
		var duetSectionButton:PsychUIButton = new PsychUIButton(objX + 100, objY, 'Duet Section', () -> {
			var side:Int = -1;
			...
			for (note in curRenderedNotes.members) {
				...
				for (i in 0...GRID_PLAYERS) {
					...
					var newNote:MetaNote = createNote(songDataCopy);
					notes.push(newNote);
		...
		var mirrorNotesButton:PsychUIButton = new PsychUIButton(objX + 200, objY, 'Mirror Notes', () -> {
			for (note in curRenderedNotes) {
				if (note == null || note.isEvent) continue;
	...
	function pasteCopiedNotesToSection(?canCopyNotes:Bool = true, ?canCopyEvents:Bool = true, ?showMessage:Bool = true):Array<MetaNote> { // Used on "Paste Section" and "Copy Last Section" buttons
		...
		if (canCopyNotes && copiedNotes.length > 0) {
			for (note in copiedNotes) {
				...
				var createdNote:MetaNote = createNote(dataCopy, curSec);
	...
	function addSongTab() {
		var tab_group:FlxSpriteGroup = mainBox.getTab('Song').menu;
		var objX:Int = 10;
		var objY:Int = 25;

		songNameInputText = new PsychUIInputText(objX, objY, 100, 'None', 8);
		songNameInputText.onChange = (old:String, cur:String) -> PlayState.SONG.song = cur;

		allowVocalsCheckBox = new PsychUICheckBox(objX, objY + 20, 'Allow Vocals', 80, () -> {
			PlayState.SONG.needsVoices = allowVocalsCheckBox.checked;
			loadMusic();
		});
		var reloadAudioButton:PsychUIButton = new PsychUIButton(objX + 120, objY, 'Reload Audio', () -> loadMusic(true), 80);

		#if mac
		var reloadJsonButton:PsychUIButton = new PsychUIButton(objX + 205, objY, 'Reload JSON', () -> {
			var cur:String = Paths.formatToSongPath(songNameInputText.text);
			var curdiff:String = Song.format(cur, PlayState.storyDifficulty);
			var diff:Bool = false;
		...
		#if mac tab_group.add(reloadJsonButton); #end

		objY += 40;
		playerDropDown = new PsychUIDropDownMenu(objX, objY, [''], (id:Int, character:String) -> {
		...
		stageDropDown = new PsychUIDropDownMenu(objX + 140, objY, [''], (id:Int, stage:String) -> {
			PlayState.SONG.stage = stage;
			StageData.loadDirectory(PlayState.SONG);
			trace('selected $stage');
		});

		opponentDropDown = new PsychUIDropDownMenu(objX, objY + 40, [''], (id:Int, character:String) -> {
		...
		girlfriendDropDown = new PsychUIDropDownMenu(objX, objY + 80, [''], (id:Int, character:String) -> {
	...
	function addFileTab() {
		var tab:PsychUITab = upperBox.getTab('File');
		var tab_group:FlxSpriteGroup = tab.menu;
		var btnX:Float = tab.x - upperBox.x;
		var btnY:Int = 1;
		...
		btnY++;
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Chart...', () -> {
		...
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Autosave...', () -> {
			if (!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			openSubState(new BasePrompt(420, hei, 'Choose an Autosave', (state:BasePrompt) -> {
				upperBox.isMinimized = true;
				upperBox.bg.visible = false;
				...
				var btn:PsychUIButton = new PsychUIButton(0, radioGrp.y + radioGrp.height + 20, 'Load', () -> {
		...
		if (SHOW_EVENT_COLUMN) {
			btnY += 20;
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Events...', () -> {
				if (!fileDialog.completed) return;
				upperBox.isMinimized = true;
				upperBox.bg.visible = false;
				...
				fileDialog.open(() -> {
					try {
						...
						openSubState(new BasePrompt('Events Found! Choose an action.', (state:BasePrompt) -> {
							var btnY:Int = 390;
							var btn:PsychUIButton = new PsychUIButton(0, btnY, 'Replace All', () -> {
								for (event in events) {
									if (event != null) {
										event.destroy();
										selectedNotes.remove(event);
		...
		btnY++;
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save', () -> {
			if (!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save as...', () -> {
			if (!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		if (SHOW_EVENT_COLUMN) {
			btnY += 20;
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save Events...', () -> {
				if (!fileDialog.completed) return;
				upperBox.isMinimized = true;

				updateChartData();
				fileDialog.save('events.json', PsychJsonPrinter.print({events: PlayState.SONG.events, format: 'psych_v1'}, ['events']), () -> showOutput('Events saved successfully to: ${fileDialog.path}'), null, () -> showOutput('Error on saving events!', true));
			}, btnWid);
			btn.text.alignment = LEFT;
			tab_group.add(btn);
		}
		...
		btnY++;
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Reload Chart', () -> {
			var func:Void->Void = () -> {
				if (Song.chartPath == null) {
		...
		btnY++;
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save (V-Slice)...', () -> {
			if (!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
					...
					openSubState(new BasePrompt('Metadata', (state:BasePrompt) -> {
						...
						var btn:PsychUIButton = new PsychUIButton(btnX, btnY, 'Save', () -> {
							...
							overwriteCheck(chartFile, '$chartName-chart.json', PsychJsonPrinter.print(pack.chart, ['events', 'notes', 'scrollSpeed']), () -> {
								overwriteCheck(metadataFile, '$chartName-metadata.json', PsychJsonPrinter.print(pack.metadata, ['characters', 'difficulties', 'timeChanges']), () -> if (overwriteSavedSomething) showOutput('Files saved successfully to: $path!'));
							});
							state.close();
						});
						...
						var textX:Float = FlxG.width / 2 - 155;
						var textY:Int = 360;
						...
						artistInput.onChange = (old:String, cur:String) -> pack.metadata.artist = cur;
						...
						charterInput.onChange = (old:String, cur:String) -> pack.metadata.charter = cur;
		...
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Psych to V-Slice...', () -> {
			if (!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			fileDialog.open('song.json', 'Open a Psych Engine Chart JSON', () -> {
				...
				openSubState(new BasePrompt('Metadata', (state:BasePrompt) -> {
					...
					var textX:Float = FlxG.width / 2 - 180;
					var textY:Int = 360;
					...
					difficultiesInput.onChange = (old:String, cur:String) -> {
		...
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  V-Slice to Psych...', () -> {
			if (!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;

			fileDialog.open('chart.json', 'Open a V-Slice Chart file', () -> {
				...
				fileDialog.open('metadata.json', 'Open a V-Slice Metadata file', () -> {
						...
						if (pack.difficulties != null) {
							fileDialog.openDirectory('Save Converted Psych JSONs', () -> {
								...
								function nextChart() {
									...
									if (pack.events != null) overwriteCheck(path + 'events.json', 'events.json', PsychJsonPrinter.print(pack.events, ['events']), () -> if (overwriteSavedSomething) showOutput('Files saved successfully to: ${fileDialog.path}!'), true);
		...
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Update (Legacy)...', () -> {
			if (!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
	...
	function addEditTab() {
		var tab:PsychUITab = upperBox.getTab('Edit');
		var tab_group:FlxSpriteGroup = tab.menu;
		var btnX:Float = tab.x - upperBox.x;
		var btnY:Int = 1;
		var btnWid:Int = Std.int(tab.width);
		...
		btnY++;
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Select All', () -> {
		...
		btn.onClick = () -> {
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			openSubState(new BasePrompt(400, 160, 'Autosave Settings', (state:BasePrompt) -> {
				var btn:PsychUIButton = new PsychUIButton(state.bg.x + state.bg.width - 40, state.bg.y, 'X', state.close, 40);
				btn.cameras = state.cameras;
				state.add(btn);
				...
				checkbox = new PsychUICheckBox(timeStepper.x + 80, timeStepper.y, 'Enabled', 60, () -> {
					autoSaveTime = 0;
		...
		btnY++;
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Clear All Notes', () -> {
			var func:Void->Void = () -> {
				resetSelectedNotes();
		...
		if (SHOW_EVENT_COLUMN) {
			btnY += 20;
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Clear All Events', () -> {
				var func:Void->Void = () -> {
					resetSelectedNotes();
	...
	function addViewTab() {
		...
		if (chartEditorSave.data.waveformColor != null) waveformSprite.color = Util.colorFromString(chartEditorSave.data.waveformColor);
		if (chartEditorSave.data.waveformAlpha != null) waveformSprite.alpha = chartEditorSave.data.waveformAlpha;

		showLastGridButton = new PsychUIButton(btnX, btnY, '', () -> {
		...
		btnY += 20;
		showNextGridButton = new PsychUIButton(btnX, btnY, '', () -> {
		...
		btnY++;
		btnY += 20;
		noteTypeLabelsButton = new PsychUIButton(btnX, btnY, '', () -> {
		...
		btnY++;
		btnY += 20;
		vortexEditorButton = new PsychUIButton(btnX, btnY, vortexEnabled ? '  Vortex Editor ON' : '  Vortex Editor OFF', () -> {
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Waveform...', () -> {
			Controls.toggleVolumeKeys(false);
			openSubState(new BasePrompt(320, 215, 'Waveform Settings', (state:BasePrompt) -> {
				upperBox.isMinimized = true;
				upperBox.bg.visible = false;
				...
				var options:Array<WaveformTarget> = [INST, PLAYER, EVERYTHING];
				var radioGrp:PsychUIRadioGroup = new PsychUIRadioGroup(check.x + 120, check.y, ['Instrumental', 'Main Vocals', 'Every Track']);
				radioGrp.cameras = state.cameras;
				radioGrp.onClick = () -> {
		...
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Go to...', () -> {
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			openSubState(new BasePrompt(420, 200, 'Go to Time/Section:', (state:BasePrompt) -> {
				...
				sectionStepper.onValueChange = () -> {
					currentSec = Std.int(sectionStepper.value);
					curTime = cachedSectionTimes[currentSec] + 0.000001;
					updateTime();
				};

				var btn:PsychUIButton = new PsychUIButton(0, timeTxt.y + 30, 'Go To', () -> {
					curSec = currentSec;
		...
		btnY++;
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Theme...', () -> {
			if (!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;

			openSubState(new BasePrompt(500, 260, 'Chart Editor Theme', (state:BasePrompt) -> {
				var btn:PsychUIButton = new PsychUIButton(state.bg.x + state.bg.width - 40, state.bg.y, 'X', state.close, 40);
				btn.cameras = state.cameras;
				state.add(btn);
		...
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Reset UI Boxes', () -> {
	...
	function updateChartData() {
		...
		for (event in events) PlayState.SONG.events.push(event.songData);
	}
	...
	function updateNotesRGB() {
		...
		for (note in strumLineNotes) note.rgbShader.enabled = !noRGBCheckBox.checked;
	}
	...
	function updateUpperBoxBg() {
		if (upperBox.selectedTab != null) {
			var menu:FlxSpriteGroup = upperBox.selectedTab.menu;
	...
	function openEditorPlayState() {
		...
		openSubState(new EditorPlayState(cast notes, vocals));
		upperBox.isMinimized = true;
	...
	override function destroy() {
		...
		for (_ => text in MetaNote.noteTypeTexts) text.destroy();
	...
	function keyDown(event:KeyboardEvent) {
		...
		if (num != -1 && FlxG.keys.checkStatus(eventKey, JUST_PRESSED)) { // note placement
			...
			var didDelete:Bool = false; // Try to find a note to delete first
			for (note in curRenderedNotes) {
				if (note == null || note.isEvent) continue;
			...
			if (!didDelete) { // If no notes were found, add a new in its place
				var didAdd:Bool = false;
		...
		if (eventKey == FlxKey.UP || eventKey == FlxKey.DOWN || page) {
			...
			if (addedNotes.length > 0) {
				if (vortexMoved) resetSelectedNotes();
				for (note in addedNotes) selectedNotes.push(note);
			...
			else nextTime = Math.floor(nextTime / snapCrochet + .002 + 1 + snapLeniency);
	...
	function keyUp(event:KeyboardEvent) {
		var num:Int = keysArray.indexOf(event.keyCode);
		if (num != -1) {
	...
	function loadFileList(mainFolder:String, ?optionalList:String = null, ?fileTypes:Array<String> = null):Array<String> {
	...
	function overwriteCheck(savePath:String, overwriteName:String, saveData:String, continueFunc:Void->Void = null, ?continueOnCancel:Bool = false) {
		if (FileSystem.exists(savePath)) {
			openSubState(new Prompt('Overwrite: "$overwriteName"?', () -> {
				overwriteSavedSomething = true;
				File.saveContent(savePath, saveData);
				if (continueFunc != null) continueFunc();
			}, continueOnCancel ? (() -> if (continueFunc != null) continueFunc()) : null));
		} else {
			overwriteSavedSomething = true;
			File.saveContent(savePath, saveData);
			if (continueFunc != null) continueFunc();
	...
	function addUndoAction(action:UndoAction, data:Dynamic) {
		function destroyFromArr(arr:Array<MetaNote>) {
			...
			for (note in arr) if (note != null) note.destroy();
		}
	...
	function redo() {
		if (isMovingNotes || currentUndo < 1) {
			FlxG.sound.play(Paths.sound('cancelMenu'), .4);
			return;
		}
