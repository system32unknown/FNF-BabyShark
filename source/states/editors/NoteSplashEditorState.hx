...
class NoteSplashEditorState extends MusicBeatState {
	...
	override function create() {
		if (imageSkin == null) imageSkin = NoteSplash.DEFAULT_SKIN + NoteSplash.getSplashSkinPostfix();
			...
			strums.forEach(function(strum:StrumNote) {
				if (FlxG.mouse.overlaps(strum)) {
					if (!FlxG.mouse.justPressed) {
						...
						if (anims > 1) for (i in 0...anims) animArray.push(strum.ID % 4 + (i * 4));
						...
						if (animArray.length > 1) {
							int = animArray[FlxG.random.bool() ? 0 : 1];
						}
