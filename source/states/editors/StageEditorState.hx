...
class StageEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent {
	...
	function addMetaTab() {
		...
		var openPreloadButton:PsychUIButton = new PsychUIButton(objX, objY, 'Preload List', () -> {
	...
	override function draw() {
		if(persistentDraw || subState == null) {
			...
			if(showSelectionQuad && spriteListRadioGroup.checkedRadio != null) {
				var spr:StageEditorMetaSprite = stageSprites[spriteListRadioGroup.labels.length - spriteListRadioGroup.checked - 1];
	...
	public function drawDebugOnCamera(spr:FlxSprite):Void {
		...
		for (num => sel in selectionSprites.members) {
			sel.setPosition(spr.x, spr.y);
			switch(num) {
				case 0: sel.setGraphicSize(Std.int(rect.width), lineSize); //Top
				case 1: //Bottom
					sel.setGraphicSize(Std.int(rect.width), lineSize);
					sel.y += rect.height - lineSize;
				case 2: sel.setGraphicSize(lineSize, Std.int(rect.height)); //Left
				case 3: //Right
